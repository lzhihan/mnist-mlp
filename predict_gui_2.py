...
class PredictApp:
    ...
    def _update_canvas(self):
        # TODO: 这里应该从画布像素读取，而不是使用 self.grid，但为了简化，暂时直接使用 self.grid 来更新显示。主画布显示预处理后的图像，但是不能裁切放大。
        """更新主画布显示预处理后的图像，放大到画布大小。"""
        arr = self._get_model_input_np()
        arr = np.clip(arr, 0.0, 1.0)
        self.canvas.delete('all')
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                ...
                x2 = x1 + CELL_SIZE
                y2 = y1 + CELL_SIZE
                self.canvas.create_rectangle(x1, y1, x2, y2, fill=hexc, outline='')

        # 绘制网格线
        for i in range(0, CANVAS_SIZE, CELL_SIZE):
            self.canvas.create_line([(i, 0), (i, CANVAS_SIZE)], fill='#222222')
            self.canvas.create_line([(0, i), (CANVAS_SIZE, i)], fill='#222222')
